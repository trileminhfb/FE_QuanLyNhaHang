<template>
  ...
  const body = {
    id_customer: 1,  // Cố định hoặc lấy từ user
    id_user: 2,      // Admin ID hoặc lấy động
    message: trimmed
  }
  ...
  try {
    ...
    adminMessage.value = ''
    await getMessages()  // Load lại tin nhắn sau khi gửi thành công
  } catch (err) {
  ...
  const getMessages = async () => {
    const res = await api.get(`/chat/get-messages/1/2`, {  // ID khách hàng và admin, lấy từ API
      headers: {
        'Authorization': `Bearer ${localStorage.getItem('auth_token')}`
      }
    })
