        ...
        <section class="form-section">
          <form @submit.prevent="submitReview" class="review-form" novalidate>
            ...
            <div class="form-group">
              ...
              <input
                id="id_food"
                v-model="formEvaluation.id_food"
  ...
  methods: {
    ...
    submitReview() {
      ...
      if (this.isEditing) {
        // Cập nhật đánh giá
        axios
          .put(`http://localhost:8000/api/client/rates/${this.editingReview.id}`, formData)
          .then((response) => {
            const updatedReview = {
        ...
        // Gửi đánh giá mới
        axios
          .post("http://localhost:8000/api/client/rates", formData)
          .then((response) => {
            const newReview = {
              id: response.data.id,
              rating: this.rating,
    ...
    deleteReview(index) {
      ...
      if (confirm("Bạn có chắc muốn xóa đánh giá này?")) {
        axios
          .delete(`http://localhost:8000/api/client/rates/${review.id}`)
          .then(() => {
