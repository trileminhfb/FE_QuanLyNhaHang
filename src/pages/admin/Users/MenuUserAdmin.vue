      ...
      <table class="w-full h-fit table-auto font-semibold text-2xl">
        <thead>
          <tr class="border-2 border-gray-300">
            ...
            <th>
              <div class="flex flex-row justify-center items-center gap-2">
                <SortButton @sort="(key) => sortBy('role', key)" />
                <p>Chức vụ</p>
              </div>
            </th>
            <th>Thao tác</th>
              ...
              <div class="h-full flex flex-row justify-start items-center">
                <div class="overflow-hidden flex flex-row justify-center items-center">
                  ...
                  <div class="flex flex-col gap-2">
                    ...
                    <div v-if="item.status === 'active'" class="ps-5">
                      <p class="bg-green-500 text-white w-44 text-center p-2 rounded-md">Đang làm việc</p>
                    </div>
                    <div v-if="item.status === 'inactive'" class="ps-5 ">
                      <p class="bg-yellow-500 text-white w-44 text-center p-2 rounded-md">Nghỉ làm</p>
                    </div>
                    <div v-if="item.status === 'banned'" class="ps-5 ">
                      <p class="bg-red-500 text-white w-44 text-center p-2 rounded-md">Bị ban</p>
                    </div>
                  </div>
                </div>
              </div>
...
const fetchUser = async () => {
  try {
    const response = await axios.get("http:// 192.168.1.53:8888/api/admin/users");
...
async function confirmDelete() {
  ...
  try {
    await axios.delete(`http:// 192.168.1.53:8888/api/admin/users/delete/${itemToDelete.value.id}`)
...
function goAdd() {
  router.push({ name: 'admin-add-users' })
}
