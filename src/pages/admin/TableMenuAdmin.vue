                ...
                <tbody>
                    <tr v-for="(item, index) in paginatedItems" :key="index" class="border-2 border-gray-300">
                        ...
                        <td class="flex justify-center items-center w-full h-full">
                            <p v-if="item.status === 1" class="bg-yellow-500 w-fit px-2">Đã đặt</p>
                            <p v-else-if="item.status === 2" class="bg-green-500 w-fit px-2">Có sẵn</p>
                            <p v-else-if="item.status === 0" class="bg-red-500 w-fit px-2">Đã đặt</p>
...
function goDetailTables() {
    router.push('/admin/tables/details-tables')
}
</script>